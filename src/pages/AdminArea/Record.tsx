...
import { publishRecord, searchRecordPagination, withdrawRecord } from "../../services/recordService";
                    ...
                    <div className="mb-1 w-full">
                        <div className="sm:flex">
                            ...
                            <div className="ml-auto flex items-center space-x-2 sm:space-x-3">
                                <Button color="primary" onClick={() => { setIsOpen(true); setType(TYPE.ADD) }}>
                                ...
                                <>
                                    ...
                                    <Table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                                        <Table.Head className="bg-gray-100 dark:bg-gray-700">
                                            ...
                                            <Table.HeadCell className="created-column">Created </Table.HeadCell>
                                            <Table.HeadCell className="action-column text-center">Actions</Table.HeadCell>
                                        </Table.Head>
                                            ...
                                            {searchResult.length > 0 ? (
                                                searchResult.map((value, index) => (
                                                    <Table.Row key={index} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                                                        ...
                                                        <Table.Cell className="whitespace-nowrap  text-base font-medium text-gray-900 dark:text-white">
                                                            {toStringDate(value.whMonthStart)} - {toStringDate(value.whMonthEnd)}
                                                        </Table.Cell>
                                                        <Table.Cell className="whitespace-nowrap  text-base font-medium text-gray-900 dark:text-white">
                                                            {value?.serialNo}
                                                        </Table.Cell>
                                                        ...
                                                        <Table.Cell>
                                                            <div className="flex items-center gap-x-2 whitespace-nowrap">
                                                                <Tooltip content="Resend email">
                                                                    <button onClick={() => handleResendEmail(value)}>
                                                                        <HiPaperAirplane className="text-xl action-btn" />
                                                                    </button>
                                                                </Tooltip>
                                                                <Tooltip content="Edit">
                                                                    <button onClick={() => {
                                                                        ...
                                                                        setRecord({
                                                                            ...value,
                                                                            whMonthStart: new Date(value.whMonthStart),
                                                                            whMonthEnd: new Date(value.whMonthEnd),
                                                                            publishedDate: new Date(value.publishedDate)
                                                                        });
                                                                    }}
                                                                        disabled={value.status === STATUS_CODE.DEACTIVE}
                                                                    >
                                                                        <HiPencil className="text-xl action-btn" />
                                                                    </button>
                                                                </Tooltip>
                                                                <Tooltip content="Publish">
                                                                    <button disabled={value.status === STATUS_CODE.ACTIVE} onClick={() => handlePublish(value.id)}>
                                                                        <HiStatusOnline className={`text-xl action-btn`} />
                                                                    </button>
                                                                </Tooltip>
                                                                <Tooltip content="Withdraw">
                                                                    <button disabled={value.status === STATUS_CODE.DEACTIVE} onClick={() => handleWithdraw(value.id)}>
                                                                        <HiReply className={`text-xl action-btn`} />
                                                                    </button>
                                                                </Tooltip>
                                                            </div>
                                                        </Table.Cell>
                                                ...
                                                <Table.Row>
                                                    <Table.Cell colSpan={10} className="text-center" >No Data Found!</Table.Cell>
                                                </Table.Row>
                                            )}
                                        </Table.Body>
                                    </Table>
